###
# 这个脚本是用来测试能否在一个函数内set完立马get，用array
###
...
agent.newVariableArrayFloat("location", 3, [4.0, 5.0, 6.0])
agent.newVariableFloat("location0")
...
def map_get(message_in: pyflamegpu.MessageNone, message_out: pyflamegpu.MessageNone):
   ...
   m = 2.0
   pyflamegpu.setVariableFloatArray3("location", 1, m)

   location = pyflamegpu.getVariableFloatArray3("location", 1)
   ...
   pyflamegpu.setVariableFloat("location0", location)
...
for agent in out_pop:
    print("value: %f"%(agent.getVariableFloat("location0")))
