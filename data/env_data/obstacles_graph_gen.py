...
def load_buildings_from_geojson(file_path: str) -> List[np.ndarray]:
    """
    从GeoJSON文件中加载建筑物数据
    """
    ...
    for feature in geojson_data['features']:
        if feature['geometry']['type'] == 'Polygon':
            ...
            # 移除最后一个重复的点（GeoJSON中多边形的最后一个点与第一个点相同）
...
def convert_to_integer_points(buildings: List[np.ndarray], scale_factor: float = 1.0) -> List[np.ndarray]:
    """
    将建筑物坐标转换为整数点
    """
    converted_buildings = []
    for building in buildings:
        # 将坐标转换为整数
        int_coords = np.round(building * scale_factor).astype(int)
        # 移除重复的点
        unique_coords = []
        for point in int_coords:
            point_tuple = tuple(point)
            if point_tuple not in unique_coords:
                unique_coords.append(point_tuple)

        if len(unique_coords) >= 3:  # 至少需要3个点形成多边形
            converted_buildings.append(np.array(unique_coords))
...
def generate_building_graph(geojson_path: str, output_path: str, margin: float = 0.1,
    ...
    print("正在转换为整数坐标...")
    int_buildings = convert_to_integer_points(buildings, scale_factor)

    print(f"转换为整数坐标后有 {len(int_buildings)} 个有效建筑物")

    # 扩展障碍物 - 使用等比例扩张方法
    expanded_obstacles = []
    for i, obstacle in enumerate(int_buildings):
        expanded = expand_obstacle_simple(obstacle, margin=margin)
...
def main():
    """
    主函数：从GeoJSON文件生成建筑物图
    """
    # 设置文件路径
    current_dir = os.path.dirname(os.path.abspath(__file__))
    geojson_path = os.path.join(current_dir, "..", "output", "transformed_buildings.geojson")
    output_path = os.path.join(current_dir, "..", "env_data", "buildings_visibility_graph.json")
